...
def get_initial_values(user):
    ...
    try:
        youngest_session = Session.objects.filter(
            project__client__user=user).order_by('date').last()
        if youngest_session is not None:
            ...
            youngest_date_with_sessions = datetime.date.today().\
                replace(day=1) - datetime.timedelta(days=1)

        first_of_that_month = youngest_date_with_sessions.replace(day=1)
        last_of_that_month = (
            first_of_that_month.replace(day=28) + datetime.timedelta(days=4)
        ).replace(day=1) - datetime.timedelta(days=1)

        # TODO rename the variables to sth now meaningful. It's not
        # "previous_month" anymore
        context = {
        ...
        last_of_prev_month = datetime.date.today().replace(day=1) - \
